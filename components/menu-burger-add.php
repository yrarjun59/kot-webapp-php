<?php
  ...
  if(isset($_POST['submit'])){
    ...
    if($itemname=="" && $itemprice==""){
      header('location:menu-burger-add.php?error=please enter itemname and price');
    }
    else if($itemname==""){
      ...
      header('location:menu-burger-add.php?error=please enter price');
        ...
        $checkDuplicateItem = "select * from `menu-item-burger` where burger_name = '$itemname' ";
        $result4 = mysqli_query($conn,$checkDuplicateItem);
        $count = mysqli_num_rows($result4);
    
        if($count>0){
        header('location:menu-burger-add.php?error=this item already added');

          }
        else {
          $sql = "insert into `menu-item-burger` (`burger_name`,`burger_price`) values('$itemname','$itemprice')";
          ...
          if(!$result){
            ...
            header('location:menu-burger.php?success=burger added successfully.....');
          }
        }
      }
    }
