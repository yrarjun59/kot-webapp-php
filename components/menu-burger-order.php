<?php
  ...
  if(isset($_POST['submit'])){
    ...
    if($tablenumber==""){
      header('location:menu-burger-order.php?error=please enter table-number&order-item-id='.$id.'');
    }
    else if(!Is_Numeric($tablenumber)){
      header('location:menu-burger-order.php?error=please enter numeric value for table-number&order-item-id='.$id.'');
    }
    else if($tablenumber>50){
      header('location:menu-burger-order.php?error=please enter the table number between 1-50&order-item-id='.$id.'');
    }
    else if($tablenumber==0 ||$tablenumber<0){
      header('location:menu-burger-order.php?error=table number cannot be zero or negative&order-item-id='.$id.'');
    }
    else {
      ...
      if(!Is_Numeric($quantity)){
        header('location:menu-burger-order.php?error=please enter numeric value&order-item-id='.$id.'');
      }
      else if($quantity==0||$quantity<0){
        header('location:menu-burger-order.php?error=quantity cannot be zero or negative&order-item-id='.$id.'');
      }
        ...
        $result = mysqli_query($conn,$sql);
          ...
          if(!$result){
            ...
            header('location:customer-bill.php?item=burger');
          }
      }
    }
